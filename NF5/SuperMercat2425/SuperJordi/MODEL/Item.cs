...
namespace SuperJordi.MODEL
{
	public class Item : IComparable<Item>
	{
		...
		public override string ToString()
		{
			...
			return $"CODE->{this.code,3} DESCRIPTION->{((description.ToString().Length >= 20) ? description.ToString().Substring(0, 18) + "." : description),-20} CATEGORY ->{((category.ToString().Length >= 20) ? category.ToString().Substring(0, 18) + "." : category),-20} STOCK ->{this.stock,-4} MIN_STOCK->{this.minStock,-6} PRICE->{this.price,-6}{currency} ON SALE ->{(onSale ? $"Y({Price}){currency}" : "N" )}";
		}
		#endregion
